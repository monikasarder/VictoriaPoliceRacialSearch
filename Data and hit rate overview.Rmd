...
of.interest <- c("Year", "Racial.appearance.abridged","Gender", "Area.type", "Unit.type", "Contact.Type" ,"Rank.of.Member", "Found",
...
trend <- dat %>%
  mutate(Racial.appearance = fct_infreq(Racial.appearance.abridged))%>%
...
lga.racial <- dat %>%
  ...
  select(`Area` = Area.type, `Local Government Area` = Local.Government.Area, `Police Service Area` = Police.Service.Area,
         `Total searches` = Total.searches,
         `Overall %`, `White %` = White, `Aboriginal %` = Aboriginal, `Middle Eastern %` = `Middle Eastern`, `Mediterranean/Mid %` = `Mediterarranean/Mid`,
         `African %`= African, `Asian %` = Asian,`South Asian %` = `South Asian`, `Pacific Islander %` = `Pacific Islander`)
